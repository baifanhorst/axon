...
figure('Name','Generating branch for pI','NumberTitle','off');
clf;
ax1=gca;
title(ax1,sprintf('Name','Generating branch for wI'));
...
figure('Name','Bifurcation diagram for pI','NumberTitle','off');
clf;
ax1=gca;
plot(ax1,par_stst(nunst_stst==0),E1_stst(nunst_stst==0),'g.',...
    ...
    par_stst(nunst_stst==4),E1_stst(nunst_stst==4),'m.',...
...
stst_lgtext={'unstable=0','unstable=2','unstable=4','hopf','fold'};
legend(ax1,stst_lgtext,'location','east');
xlabel(ax1,'pI');
ylabel(ax1,'E1');
title(ax1,sprintf('Bifurcation diagram for pI'));