...
figure_name = sprintf("pE pI N=%g", N);
% Title name
title_name = sprintf("pE pI N=%g", N);
...
% t_start = 100;
% t_end = 200;
% freq_mat = zeros(length(par1_list), length(par2_list));
% for i=1:length(par1_list)
%     for j=1:length(par2_list)
%         pE = par1_list(i)
%         pI = par2_list(j)
%         
...
% end
% 
% [X,Y] = meshgrid(par1_list, par2_list);
% figure(fig_2par);
% ax = gca;
% hold on;
