...
% %% Initialize the branch manually
% stst_branch0 = df_brnch(funcs, contpar, 'stst');
% stst_branch0.parameter.min_bound = [contpar, 0.001];
% stst_branch0.parameter.max_bound = [contpar, 1];
% stst_branch0.parameter.max_step = [contpar, 0.005];
% stst_branch0.point = stst;
% 
% stst2 = stst;
% stst2.parameter(contpar) = stst2.parameter(contpar)+0.005;
...
[stst_branch0] = br_contn(funcs,stst_branch0,200);
...
legend(ax1,stst_lgtext,'location','best');
xlabel(ax1,namepar);
ylabel(ax1,'E1');
title(ax1,sprintf(append('Generating branch for ', namepar)));
