...
figure('Name','Generating branch for wE','NumberTitle','off');
clf;
ax1=gca;
title(ax1,sprintf('Generating branch for wE'));
...
par_stst=getpar(stst_branch_wbifs,ind_wE);
E1_stst=getx(stst_branch_wbifs,1);
figure('Name','Bifurcation diagram for wE','NumberTitle','off');
clf;
ax1=gca;
plot(ax1,par_stst(nunst_stst==0),E1_stst(nunst_stst==0),'g.',...
    ...
    bgetpar(stst_branch_wbifs,ind_wE,'hopf'),bgetx(stst_branch_wbifs,1,'hopf'),'ks',...
    bgetpar(stst_branch_wbifs,ind_wE,'fold'),bgetx(stst_branch_wbifs,1,'fold'),'mo');
stst_lgtext={'unstable=0','unstable=1','unstable=2','hopf','fold'};
legend(ax1,stst_lgtext,'location','east');
xlabel(ax1,'wE');
ylabel(ax1,'E1');
title(ax1,sprintf('Bifurcation diagram for wE'));